...
//
// Ported verbatim from worker/templates/customerEmail.js — see
// src/pages/api/submit-form.ts for why.
...
// ── Main builder ─────────────────────────────────────────────────────────
//
// `data` shape is the same object the static site sends — see
// clientEmail.js for the field list.
export function buildCustomerEmailHtml(data) {
  ...
  // WE'LL CONFIRM BY WHATSAPP — only promise to ask for details the customer
  // DIDN'T already provide on the form (flight number for airports, exact
  // drop-off address for city drop-offs). If they filled them in, skip.
  {
    const items = [];
    if (isAirport && !data.flightNumber) items.push('your flight number');
    if (isCityDropoff && !data.dropoffAddress) items.push('your exact drop-off address');
    if (items.length) {
      adviceBlocks += adviceBlock({
        icon: '&#128241;',
        title: 'WE&rsquo;LL CONFIRM A FEW DETAILS BY WHATSAPP',
        paragraphs: [
          `To finalize your ride, we&rsquo;ll message you on WhatsApp to confirm ${items.join(' and ')}.`,
          'You can also reach us anytime at +33 6 34 30 12 92.',
        ],
      });
    }
  }
  ...
  if (isAirport) {
    adviceBlocks += adviceBlock({
      ...
      paragraphs: [
        'The driver tracks your flight and adjusts if it arrives early or late.',
  ...
  if (isRoundTrip) {
    adviceBlocks += adviceBlock({
      ...
      // one place — src/config/prices.js on the site — and this endpoint
      // deploys separately, so quoting a number here would be a second copy
      // that silently goes stale the day the rate changes.
      paragraphs: [
  ...
  // CLOSING NOTE — friendly thank-you + WhatsApp. Always shown, sits just
  // above the "NEED TO REACH US?" contact section.
  body += `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="width:100%; border-collapse:collapse; margin-top:24px;">
    <tr><td style="padding-top:0; padding-right:0; padding-bottom:0; padding-left:0;">
      <p style="margin-top:0; margin-right:0; margin-bottom:12px; margin-left:0; font-family:${FONT_BODY}; font-size:14px; color:${COLORS.ink}; line-height:22px; mso-line-height-rule:exactly;">Thank you for choosing our service and for placing your trust in us. We are pleased to confirm that your reservation has been successfully booked.</p>
      <p style="margin-top:0; margin-right:0; margin-bottom:12px; margin-left:0; font-family:${FONT_BODY}; font-size:14px; color:${COLORS.ink}; line-height:22px; mso-line-height-rule:exactly;">Please note that not all of our drivers are equipped with a card payment terminal, so if possible, we kindly recommend having some cash with you. If that&rsquo;s not possible, no worries&mdash;we&rsquo;ll always find a suitable solution.</p>
      <p style="margin-top:0; margin-right:0; margin-bottom:12px; margin-left:0; font-family:${FONT_BODY}; font-size:14px; color:${COLORS.ink}; line-height:22px; mso-line-height-rule:exactly;">If you have any questions before your journey, please don&rsquo;t hesitate to contact us on WhatsApp at <a href="https://wa.me/33634301292" style="color:${COLORS.accent}; text-decoration:none; font-weight:bold;">+33 6 34 30 12 92</a>.</p>
      <p style="margin-top:0; margin-right:0; margin-bottom:0; margin-left:0; font-family:${FONT_BODY}; font-size:14px; color:${COLORS.ink}; line-height:22px; mso-line-height-rule:exactly;">Thank you again for your reservation. We wish you a pleasant trip and look forward to welcoming you soon!</p>
    </td></tr>
  </table>`;
  ...
  body += `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="width:100%; border-collapse:collapse; margin-top:28px; border-top-width:1px; border-top-style:solid; border-top-color:${COLORS.smoke};">
    <tr><td style="padding-top:20px; padding-right:0; padding-bottom:0; padding-left:0;">
      ...
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="width:100%; border-collapse:collapse;">
        ...
        ${linkRow('Email', 'thedriver.france@gmail.com', 'mailto:thedriver.france@gmail.com')}
