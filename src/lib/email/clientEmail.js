...
//
// Ported verbatim from worker/templates/clientEmail.js when the booking
// backend moved off the (now-inaccessible) Cloudflare account onto a Vercel
// API route — see src/pages/api/submit-form.ts. No logic changed.
//
...
const L = {
  ...
  toConfirm:      '&Agrave; CONFIRMER AVEC LE CLIENT',
  dropoffAddrLbl: 'Adresse d&rsquo;arriv&eacute;e',
  toAsk:          '&Agrave; demander par WhatsApp',
};
...
// Pickup/dropoff categories — used to decide which "to confirm with the
// customer" reminders to show. Flight number (airport pickups) and exact
// destination address (city drop-offs) are no longer collected on the form;
// the chauffeur asks for them via WhatsApp.
...
export function buildClientEmailHtml(data) {
  ...
  // TO CONFIRM WITH CUSTOMER — flight number (airport pickups) and exact
  // destination address (city drop-offs) aren't collected on the form
  // anymore. Flag them here so the chauffeur knows to ask via WhatsApp.
  {
  ...
  // SHARE-READY SUMMARY — stacked, emoji-prefixed lines the chauffeur can
  // select and copy-paste straight into WhatsApp to forward to another driver.
  // Sits below everything, just above the footer.
  {
    const dParts = String(data.date || '').split('-'); // YYYY-MM-DD
    const dateShort = dParts.length === 3 ? `${dParts[2]}/${dParts[1]}` : esc(data.date || '—');
    const pickupCopy = data.pickupAddress
      ? esc(data.pickupAddress)
      : (data.pickupStation ? `${esc(data.pickup)} (${esc(data.pickupStation)})` : esc(data.pickup || '—'));
    ...
    body += `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="width:100%; border-collapse:collapse; margin-top:24px; border-top-width:1px; border-top-style:solid; border-top-color:${COLORS.smoke};">
      <tr><td style="padding-top:20px; padding-right:0; padding-bottom:0; padding-left:0;">
        ${sectionHead('&Agrave; PARTAGER (COPIER-COLLER)')}
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="width:100%; border-collapse:collapse; background-color:${COLORS.cream};">
          <tr><td style="padding-top:14px; padding-right:16px; padding-bottom:14px; padding-left:16px; font-family:${FONT_BODY}; font-size:14px; color:${COLORS.ink}; line-height:24px; mso-line-height-rule:exactly;">${shareLines}</td></tr>
        </table>
      </td></tr>
    </table>`;
  }
